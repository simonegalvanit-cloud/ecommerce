...
export default function StorefrontPage() {
  ...
  const addToCart = () => {
    const itemId = `${modalProduct!.key}-${SIZES[selSizeIdx]?.label}`
    setCart(prev => {
    ...
    <>
      ...
      <aside className={`cart-drawer${cartOpen ? ' open' : ''}`} aria-label="Carrello">
        <div className="cart-drawer-head">
          ...
          <button className="cart-drawer-close" onClick={() => setCartOpen(false)} aria-label="Chiudi carrello">
            <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24"><path d="M18 6L6 18M6 6l12 12"/></svg>
          </button>
        </div>
        ...
        <div className="cart-drawer-body">
          {cart.length === 0 ? (
            <div className="cart-empty">
              <svg width="52" height="52" fill="none" stroke="currentColor" strokeWidth="1.2" viewBox="0 0 24 24" opacity=".25"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
              ...
              {cart.map(item => (
                <li key={item.id} className="cart-item">
                  ...
                  <div className="cart-item-right">
                    ...
                    <button className="cart-item-remove" onClick={() => removeFromCart(item.id)} aria-label="Rimuovi">
        ...
        {cart.length > 0 && (
          <div className="cart-drawer-foot">
            ...
            <button className="cart-continue-btn" onClick={() => setCartOpen(false)}>Continua a comprare</button>
          </div>
        )}
      </aside>
